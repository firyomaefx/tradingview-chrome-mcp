/**
 * Runtime feature flags for the hosted fork.
 */
...
async function loadFromSupabase(): Promise<Partial<FeatureFlags>> {
  if (!supabase) return {};
  const { data, error } = await supabase.from("mcp_feature_flags").select("key, value");
...
export async function loadFeatureFlags(): Promise<FeatureFlags> {
  const overrides = config.telemetryEnabled ? await loadFromSupabase() : loadFromEnv();
